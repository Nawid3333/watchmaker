...
Series that the preview finds already at the target state are reported as-is and are not touched again, so a re-run only does work where something actually has to change.
